...
def view_class(request, pk=None):
    if pk is None:
        ...
        for student in students:
            student.total_class = (
                student.get_total_class()
            )  # Calculate total class for each student
        return render(
            request,
            "class_details.html",
            {"class": _class, "students": students, "page_title": "Class Information"},
        )
        ...
        if student_id:
            try:
                with transaction.atomic():
                    ...
                    remaining_students = Student.objects.all().order_by("id")
                    for index, student in enumerate(remaining_students, start=1):
                        student.id = index
                        student.save(update_fields=["id"])

                    resp["status"] = "success"
                    resp["msg"] = "Student details have been deleted successfully."
            except Exception as e:
                resp["msg"] = "An error occurred while deleting the student."
                print(e)  # Log the exception for debugging
        else:
            resp["msg"] = "Student ID not provided."
    else:
