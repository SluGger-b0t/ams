...
class ClassStudent(models.Model):
    ...
    def get_present(self):
        ...
        try:
            present = Attendance.objects.filter(
                classIns=_class, student=student, type=2
            ).count()
            return present
        except:
            return 0
    ...
    def get_absent(self):
        ...
        try:
            present = Attendance.objects.filter(
                classIns=_class, student=student, type=3
            ).count()
            return present
        except:
            return 0

    def get_total_class(self):
        return self.get_present() + self.get_absent()
    
    def get_percentage(self):
        try:
            return self.get_present()/self.get_total_class()*100
        except:
            return 0
...
class Attendance(models.Model):
    ...
    type = models.CharField(
        max_length=250, choices=[("1", "Present"), ("2", "Tardy"), ("1", "Absent")]
    )
    date_updated = models.DateTimeField(auto_now=True)
